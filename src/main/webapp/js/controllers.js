...
function DocRoot($scope, $location, $localStorage, $filter, $http) {
    ...
    $scope.visitWebsiteForProVersion = 'Visit our <a href="http://www.miredot.com/price/?licencerequest=pro">website</a> to get the full version (free for open source).';
    ...
    $scope.projectName = com.qmino.miredot.restApiSource.projectName;
    $scope.projectVersion = com.qmino.miredot.restApiSource.projectVersion;
    ...
    $scope.versionCheckResult = $http.jsonp(
        $scope.restBase +
        'version' +
        ...
        .success(function(data) {
            if (data.upToDate) {
                $scope.versionCheckResult = " version " + data.version;
            } else {
    ...
    $scope.serviceTags = (function() {
        ...
        return tagNames.map(function(tagName) { return {name : tagName, selected: false }; } );
    }());
    ...
    $scope.interfaceHttpOrderFunction = function(iface) {
        return $scope.httpMethods.indexOf(iface.http);
    };

    $scope.methodHttpOrderFunction = function(method) {
        return $scope.httpMethods.indexOf(method.method);
    };
